...
from collbank.basic.models import UserSearch
...
class BasicList(ListView):
    ...
    def get_data(self, prefix, dtype, response=None):
        ...
        try:
            ...
            # Create header cells
            headers = [x['name'] for x in specification if x['type'] != "" and x['type'] in ['func', 'field'] ]
            headers.insert(0, "Id")
            for col_num in range(len(headers)):
                c = ws.cell(row=1, column=col_num+1)
                c.value = headers[col_num]
...
class BasicDetails(DetailView):
    """Extension of the normal DetailView class for this application"""
