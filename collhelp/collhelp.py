...
# ----------------------------------------------------------------------------------
# Name :    collhelp
# Goal :    Create fixtures for the 'help' part of the collection bank
# History:
# 27/jun/2016    ERK Created
# ----------------------------------------------------------------------------------
def collhelp(csv_file, output_file):
    try:
            ...
            if (strLine != ""):
                ...
                if (not bFirst):
                    ...
                    for i in range(len(arPart)):
                        if arFieldNames[i] != 'pk':
                            if arFieldNames[i] == 'searchable':
                                oFields[arFieldNames[i]] = (arPart[i] == '1')
                            else:
                                oFields[arFieldNames[i]] = arPart[i]

                    # Expected fields: [pk], [searchable], [field], [display_name], [help_url]
